...
import { Menu, X, Calendar, Home, Sparkles, Grid3x3, DollarSign, Users, MapPin } from 'lucide-react';
...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className="w-full max-w-7xl px-5 flex justify-center">
          <nav className="relative flex items-center justify-between glass border border-sage-200/30 shadow-lg w-full md:w-auto md:min-w-[1000px] rounded-2xl md:rounded-full py-1 pl-4 pr-4">
            ...
            >
              ...
              <span className="font-serif font-semibold text-2xl text-charcoal">
                KAPSUL
              </span>
            </Link>
            ...
            <div className="hidden md:flex items-center gap-1.5 bg-sage-50/40 p-2 rounded-full border border-sage-100/50">
              {navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    <Icon 
                      className={`w-4 h-4 transition-all duration-300 ${isActive ? 'text-sage-600' : 'text-gray-500 group-hover:text-sage-600 group-hover:scale-110'}`}
      ...
      {/* MOBILE MENU */}
      <div 
        className={`fixed inset-0 z-60 md:hidden transition-all duration-500 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      >
        <div 
        ...
        <div 
          className={`absolute top-2 right-2 bottom-2 w-full max-w-[340px] bg-white rounded-3xl shadow-2xl flex flex-col overflow-hidden transform transition-transform duration-500 ease-out ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-[110%]'}`}
        >
          {/* Header */}
          <div className="flex items-center justify-between p-6 border-b border-sage-100">
            <span className="font-serif font-semibold text-xl text-charcoal">Menu</span>
            <button 
              onClick={closeMobileMenu}
            ...
            {navLinks.map((link, index) => {
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  onClick={closeMobileMenu}
                  className={`group flex items-center gap-4 p-4 rounded-xl transition-all ${isActive ? 'bg-sage-50 shadow-sm' : 'hover:bg-sage-50/50'}`}
                  style={{ 
                ...
                >
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-all ${isActive ? 'bg-sage-600 text-white shadow-sm' : 'bg-sage-100/50 text-sage-600 group-hover:bg-sage-600 group-hover:text-white group-hover:scale-110'}`}>
                    <Icon className="w-5 h-5" strokeWidth={isActive ? 2.5 : 1.5} />
                  </div>
                  <span className={`font-semibold ${isActive ? 'text-sage-700' : 'text-charcoal'}`}>
                    {link.name}
                  </span>
                </Link>
              );
